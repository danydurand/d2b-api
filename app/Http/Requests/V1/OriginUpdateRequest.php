...
class OriginUpdateRequest extends FormRequest
{
    /**
    ...
    {
        $method = $this->method();
        $id     = Route::current()->parameter('origin')->id;

        if ($method == 'PUT') {
            return [
                'description'  => ['required', 'string', 'max:100', Rule::unique('origins')->ignore($id)],
            ...
            return [
                'description'  => ['sometimes', 'required', 'string', 'max:100', Rule::unique('origins')->ignore($id)],
