...
class DocumentTypeUpdateRequest extends FormRequest
{
    ...
    {
        $method = $this->method();
        $id     = Route::current()->parameter('document_type')->id;

        if ($method == 'PUT') {
            return [
                'description'  => ['required', 'string', 'max:100', Rule::unique('document_types')->ignore($id)],
            ...
            return [
                'description'  => ['sometimes', 'required', 'string', 'max:100', Rule::unique('document_types')->ignore($id)],
