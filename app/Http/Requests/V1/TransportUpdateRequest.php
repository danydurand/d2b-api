...
class TransportUpdateRequest extends FormRequest
{
    ...
    {
        $method = $this->method();
        $id     = Route::current()->parameter('transport')->id;

        if ($method == 'PUT') {
            return [
                'code'         => ['required', 'string', 'max:6', Rule::unique('transports')->ignore($id)],
                'name'         => ['required', 'string', 'max:100', Rule::unique('transports')->ignore($id)],
            ...
            return [
                'code'         => ['sometimes', 'required', 'string', 'max:6', Rule::unique('transports')->ignore($id)],
                'name'         => ['sometimes', 'required', 'string', 'max:100', Rule::unique('transports')->ignore($id)],
