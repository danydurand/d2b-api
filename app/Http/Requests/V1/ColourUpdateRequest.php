...
class ColourUpdateRequest extends FormRequest
{
    ...
    {
        $method = $this->method();
        $id     = Route::current()->parameter('colour')->id;

        if ($method == 'PUT') {
            return [
                'description'  => ['required', 'string', 'max:100', Rule::unique('colours')->ignore($id)],
            ...
            return [
                'description'  => ['sometimes', 'required', 'string', 'max:100', Rule::unique('colours')->ignore($id)],
